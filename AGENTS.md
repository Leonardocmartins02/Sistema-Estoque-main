...
**Revisão final de Ajuste de Estoque (28/08/2026)** — a feature passou pelos reviewers de segurança e acessibilidade sem achados bloqueantes. Ficaram registrados **dois riscos aceitos** (#7 o histórico expõe o e-mail do responsável, hoje o único identificador humano que `User` possui; #10 a agregação de `ADJUSTMENT` acontece em memória e pode não escalar com volume) e **seis dívidas** (A1 foco entre os passos do `AdjustmentFormModal`; A4 região `aria-live` do preview no fluxo de revisão; A5 a seta `→` do histórico pode não ser anunciada por leitor de tela; A6 `role="alert"` inconsistente entre campos; #8 a resposta do histórico espalha o registro cru em vez de um DTO mínimo explícito; #11 `ON DELETE SET NULL` em `userId` reduziria a rastreabilidade se exclusão de usuário for introduzida). Nenhum é bug em produção e nenhum tem prioridade atribuída. Contexto, impacto e correção proposta de cada um: `docs/features/ajuste-estoque/review.md`.
