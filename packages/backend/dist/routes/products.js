...
router.get('/', async (req, res) => {
    ...
    const sortBy = ['name', 'sku', 'balance'].includes(sortByRaw) ? sortByRaw : 'name';
    const sortDir = sortDirRaw === 'desc' ? 'desc' : 'asc';
    const statusParam = String(req.query.status || '').trim();
    const statusFilter = statusParam
        ? statusParam.split(',').map((s) => s.toUpperCase()).filter((s) => ['OK', 'ATTN', 'OUT'].includes(s))
        : [];
    ...
    const productsWithBalance = await Promise.all(products.map(async (p) => {
    ...
    const filteredByStatus = statusFilter.length === 0
        ...
        : productsWithBalance.filter((p) => {
            ...
            const map = { OK: isOk, ATTN: isAttn, OUT: isOut };
    ...
    try {
        ...
        if (data.sku) {
            ...
            if (existing && existing.id !== id)
                return res.status(409).json({ message: 'SKU já em uso.' });
        }
